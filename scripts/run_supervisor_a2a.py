#!/usr/bin/env python3
"""
Run script for Supervisor A2A server.

This script starts the Supervisor agent with A2A protocol support.
...
def main():
    """Main entry point for running Supervisor A2A server."""
    try:
        ...
        agent_config = a2a_config.get_agent_config("supervisor")
        
        if not agent_config or not agent_config.enabled:
            logger.error("Supervisor agent is not enabled in a2a.config.yml")
            sys.exit(1)
        
        # Get supervisor agent configuration
        supervisor_info = project_config.agents.supervisor
        if not supervisor_info:
            logger.error("Supervisor configuration not found in project config")
            sys.exit(1)
        
        # Create Supervisor agent using the same approach as Team class
        logger.info("Creating Supervisor agent...")
        supervisor = SupervisorAgent(
            id=supervisor_info.agent_id,
        ...
        app = create_supervisor_a2a_app(supervisor, db_path)
        
        # Run the server
        logger.info(f"Starting Supervisor A2A server on {agent_config.host}:{agent_config.port}")
        uvicorn.run(
            app,
        ...
        logger.info("Shutting down Supervisor A2A server...")
    except Exception as e:
        logger.error(f"Failed to start Supervisor A2A server: {str(e)}", exc_info=True)
        sys.exit(1)
