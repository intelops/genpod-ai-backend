#!/usr/bin/env python3
"""
Run script for Reviewer A2A server.
"""
...
def main():
    """Main entry point for running Reviewer A2A server."""
    try:
        ...
        agent_config = a2a_config.get_agent_config("reviewer")
        
        if not agent_config or not agent_config.enabled:
            logger.error("Reviewer agent is not enabled in a2a.config.yml")
            sys.exit(1)
        
        # Get reviewer agent configuration
        reviewer_info = project_config.agents.reviewer
        if not reviewer_info:
            logger.error("Reviewer configuration not found in project config")
            sys.exit(1)
        
        # Create Reviewer agent
        logger.info("Creating Reviewer agent...")
        reviewer = ReviewerAgent(
            id=reviewer_info.agent_id,
            name=reviewer_info.agent_name,
            description=reviewer_info.description,
            llm=reviewer_info.llm,
            recursion_limit=reviewer_info.recursion_limit,
            persistence_db_path=db_path,
            use_rag=reviewer_info.use_rag
        )

        # Create A2A app
        logger.info("Creating A2A application...")
        app = create_reviewer_a2a_app(reviewer, db_path)
        
        # Run the server
        logger.info(f"Starting Reviewer A2A server on {agent_config.host}:{agent_config.port}")
        uvicorn.run(
            app,
        ...
        logger.info("Shutting down Reviewer A2A server...")
    except Exception as e:
        logger.error(f"Failed to start Reviewer A2A server: {str(e)}", exc_info=True)
        sys.exit(1)
