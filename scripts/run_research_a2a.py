#!/usr/bin/env python3
"""
Run script for Research A2A server.

This script starts the Research agent with A2A protocol support.
...
def main():
    """Main entry point for running Research A2A server."""
    try:
        ...
        agent_config = a2a_config.get_agent_config("research")
        
        if not agent_config or not agent_config.enabled:
            logger.error("Research agent is not enabled in a2a.config.yml")
            sys.exit(1)
        
        # Get research agent configuration
        research_info = project_config.agents.research
        if not research_info:
            logger.error("Research configuration not found in project config")
            sys.exit(1)
        
        # Create Research agent using the same approach as Team class
        ...
        app = create_research_a2a_app(research, db_path)
        
        # Run the server
        logger.info(f"Starting Research A2A server on {agent_config.host}:{agent_config.port}")
        uvicorn.run(
            app,
        ...
        logger.info("Shutting down Research A2A server...")
    except Exception as e:
        logger.error(f"Failed to start Research A2A server: {str(e)}", exc_info=True)
        sys.exit(1)
