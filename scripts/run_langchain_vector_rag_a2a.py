#!/usr/bin/env python3
"""
Run script for LangChain Vector RAG A2A server.

This script starts the LangChain Vector RAG agent with A2A protocol support.
...
def main():
    """Main entry point for running LangChain Vector RAG A2A server."""
    try:
        ...
        agent_config = a2a_config.get_agent_config("langchain_vector_rag")
        ...
        # Create LangChain Vector RAG agent using the same approach as Team class
        logger.info("Creating LangChain Vector RAG agent...")
        rag_agent = RAGAgent(
        ...
        app = create_langchain_vector_rag_a2a_app(rag_agent, db_path)
        
        # Run the server
        logger.info(f"Starting LangChain Vector RAG A2A server on {agent_config.host}:{agent_config.port}")
        uvicorn.run(
            app,
        ...
        logger.info("Shutting down LangChain Vector RAG A2A server...")
    except Exception as e:
        logger.error(f"Failed to start LangChain Vector RAG A2A server: {str(e)}", exc_info=True)
        sys.exit(1)
